...
// half of decision 4 is wired in separately.
func ResolveBlockedItemsForArticle(ctx context.Context, q *db.Queries, groupID string, articleID pgtype.UUID, checkDate time.Time) (bool, error) {
...
func ResolveOverdueSwaps(ctx context.Context, q *db.Queries) (int, error) {
	...
	for _, item := range items {
		...
		ok, err := ResolveBlockedItemsForArticle(ctx, q, item.GroupID, item.ArticleID, time.Now())
		if err != nil {
