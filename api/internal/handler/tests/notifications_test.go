...
func mountNotifRoutes(env *testutil.TestEnv, notifier *notifications.CapturingNotifier) {
	env.V1(func(r chi.Router) {
		...
		r.Mount("/bookings", (&handler.BookingHandler{Q: env.Queries, Pool: env.Pool, Perms: handler.NewPermissionCache(env.Queries), Notifier: notifier, BaseURL: "http://test.example"}).Routes())
