...
6. **Condition-change triggers (`reported_usable`/`reported_unusable`/`missing`).**
   - `reported_usable` (damaged but still bookable - article status stays in the available set): **opportunistic upgrade only**. If a fully-`ok` equivalent unit is available for the waiting booking, swap it in; otherwise leave the still-usable unit assigned. Never notifies - nothing is actually blocked, since a `reported_usable` article remains bookable.
...
7. **`Update`'s conflict path also swaps before it 409s, universally - not copy-only.** Today, changing a booking's dates (`Update` in `api/internal/handler/bookings.go`) hard-conflicts (`409 article_not_available_for_dates`/`articles_not_available_for_dates`) if any currently-held item isn't available for the new range. This is the same shape of problem as the other triggers - the user doesn't care which physical unit they end up with - so `Update` now tries `FindEquivalentAvailableArticle` per conflicting item first and silently swaps (logging a `swap` booking_event) before falling back to the 409 for whatever's left unresolved. This is what makes "copy a booking, get new dates, it just works" true even when the exact original items are unavailable - Copy needs no special-casing, since it already goes through `Update`.

4. **Notification target for "no swap available": same pattern as archive warnings** - broadcast to the waiting booking's team channels (email/GChat, respecting Gruppkanal prefs) *and* a personal email to the booking's creator, deduped via `notification_log`. New `EventKey` `booking_item_blocked` (or similar), reusing the existing generic booking-email template (subject/banner/CTA/intro all keyed off `email_*_<event>` i18n keys - no template code changes needed, just new translation keys + an `eventStyles` entry).
...
- **Nightly job:** enumerates `booking_items` still `picked_up` where the booking's `end_date` has passed and `return_status` is `NULL` (never explicitly marked) or `delayed` (marked but not yet resolved). `check_date` = today, since there's no manager-entered estimate to look further ahead with.
...
- If no equivalent exists: send the "no swap available" notification (decision 4), naming the affected item(s) but not the current (late) booker, per the doc's original privacy note. Skipped entirely for the `reported_usable` upgrade-only case.
...
- `ResolveBlockedItemsForArticle(ctx, q, groupID, articleID, checkDate) (swapped bool, err error)` - one article, called synchronously from the mark-delayed handler (`UpdateBookingItemReturnStatus`'s `delayed` case in `bookings.go`). When no equivalent unit is found, currently a silent no-op - the "notify instead" half of decision 4 is a later phase, not yet wired.
- `ResolveOverdueSwaps(ctx, q) (swapped int, err error)` - the nightly entry point, wired into `runBookingCleanupJobs` in `main.go`. Notification count/wiring deferred to the same later phase as above.
