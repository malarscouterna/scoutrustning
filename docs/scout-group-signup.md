...
6. **Frontend: own-profile actions in `UserInfoCard` + dashboard-root affordance.** When the card is opened for your own user ID (not someone else's), add: a **settings link** to `/profile`; a **sign-out** button/form reusing the existing `POST /auth/signout` pattern already on `profile/+page.svelte:1140`; and the **group switcher** (only rendered when the member has more than one registered org), which sets the `active-group-id` cookie and reloads. Also surface the same switcher (or a link into the popup) from `/` for discoverability. `+layout.server.ts` reads the resolved active group same as it reads `data.user` today.
