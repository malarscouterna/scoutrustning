...
3. **Migration: composite `users` PK.** `goose` migration changing `users` PK from `id` to `(id, group_id)`; add composite unique `(id, group_id)` if PK syntax needs it split out; update every FK currently `REFERENCES users(id)` (enumerate via `\d users` / grep on `REFERENCES users` across `migrations/`) to composite `(user_id, group_id) REFERENCES users(id, group_id)`. No data backfill needed - existing rows already have both columns populated, this only tightens/reshapes constraints. Regenerate sqlc models (`users.id` becomes part of a composite key in generated Go structs - check fallout in `internal/db/`). No behavior change yet; existing single-group flows must still pass the full test suite unmodified.
...
4. Group switching (largest scope: `users` composite-PK migration, FK updates, `/api/v0/me` multi-group response, active-group switcher endpoint, UI switcher) + bundled nav discoverability fixes (§1). **Current phase.**
