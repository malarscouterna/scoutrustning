...
### CSV import - hardcoded Mälarscouterna "Karsvik" special case wrote to the wrong column, and Swedish column names

Found while writing inline CSV import help text (item 15, below), then verified against a real export (`tmp/import-data-gillis.csv`, 320 rows). `api/internal/handler/articles.go`'s `Import` handler had a group-agnostic hardcoded special case with no `group_id` scoping: if `location == "Karsvik"` (literal string) and `plats` was set, `plats` (aliased through a Mälarscouterna-specific spelling table: `"ostergarden"` → `"Östergården"`, etc.) *replaced* `location` outright - discarding the site name from the result and making every distinct `plats` value its own top-level browsable location. Confirmed genuinely load-bearing against the real file (110/320 rows depend on it), not dead code - but still wrong: `location`/grouping should answer "which geographical site" (the only structured, filterable place field - `articles.location_id`), while `plats` answers a finer question one level down that belongs in `articles.place` (free text, already existed for exactly this, previously only fed by `rum`+`lage`).

**Fixed in this change:**
- `plats` (now composed together with `rum`/`lage`) joins `place`, never touches `location`. The `EqualFold(rawLocation, "Karsvik")` gate and the `locationName` override are gone entirely - no hardcoded site name survives anywhere. `normalizeKarsvikPlats` renamed to `normalizePlats` (still Mälarscouterna-specific spelling normalization, but no longer gated behind a literal group name, and harmless for other groups since unmatched input passes through unchanged).
- **CSV column contract renamed to English**, matching the project's own domain terms: `titelgrupp`→`commercial_name`, `title`→`common_name`.
- **Canonical format maps 1:1 to the actual DB shape** (`articles.location_id` + `articles.place`), not to an aspirational 4-level hierarchy the DB doesn't have. A new `place` column is read directly when present. The old 3-way `plats`/`rum`/`lage` split is kept **only** as a legacy fallback (used when `place` is absent) - it still composes into one string internally exactly as before, but is no longer the documented/promoted shape.
- `docs/import-example.csv` rewritten to the canonical `commercial_name, common_name, ..., location, place, tags, requires_approval` header, with `plats`/`rum`/`lage` values pre-combined into `place` for every existing row.
- Item 15's docs updated to match: the `/settings` inline help panel shows only the canonical columns (no legacy-format mention - it's a quick reference for building a new file); `docs/guide.md`'s column reference does the same but adds one line noting `plats`/`rum`/`lage` still work for old exports, since that's exactly the kind of thing a deeper reference is for.
- Two new integration tests in `TestArticleCSVImport` (`api/internal/handler/tests/articles_test.go`): canonical `place` maps directly with `location` unchanged; legacy `plats`/`rum`/`lage` still composes into `place` with `location` unchanged. Full suite (`go test ./internal/handler/tests/`) passes.

**Caught by manually running the import against the real `tmp/import-data-gillis.csv` file (320 rows) after implementing:** the first pass genuinely broke that file - 0 imported, 320 skipped. The initial reasoning ("no legacy aliasing needed for `title`/`titelgrupp` - nobody depends on the old names") turned out to be wrong: this real historical file uses `title`/`titelgrupp` *and* `plats`/`rum`/`lage` together, the same era's format, and only the location-split columns had a fallback path. Added a `colAny()` helper (`common_name` falls back to `title`; `commercial_name` falls back to `titelgrupp`) for consistency with the `place` fallback already in place. Re-ran against the same file: **1020 imported, 0 skipped, no errors** - verified directly in the DB afterward that only 4 real locations exist (matching the file's 4 actual sites) and that Karsvik-site articles show composed `place` values like `"Östergården, Mötesförrådet, Knivar"` with `location` staying exactly `"Karsvik"`.
