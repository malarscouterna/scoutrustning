...
- [ ] Rename `/profile` route to `/settings` - the page mixes personal profile (`profile` tab) with group/team administration (`teams`, `group` tabs: locations, categories, CSV import, language, permission roles, notifications, danger zone). "Profile" undersells what's actually a settings page for managers. Stays one page, three tabs, same structure - just the route path and any nav labels/links pointing at `/profile` change.
