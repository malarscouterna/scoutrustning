...
11. ~~`feat(api,web): delayed return - auto-swap, conflict overview, next-booker notification`~~ - Done. Scope grew (2026-07-08) to also cover condition-change swaps (`reported_usable`/`reported_unusable`/`missing`) and universal swap-before-409 in `Update`'s conflict path (which also fixed item 10's copy-flow gap above for free). Full design and decision log in `docs/delayed-return-swap.md`.
