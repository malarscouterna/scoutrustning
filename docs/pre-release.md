...
11. `feat(api,web): delayed return - auto-swap, conflict overview, next-booker notification` - Auto-swap logic, booking page conflict section, notification without names. Depends on 6. Scope grew (2026-07-08) to also cover condition-change swaps (`reported_usable`/`reported_unusable`/`missing`) and universal swap-before-409 in `Update`'s conflict path (which also fixes item 10's copy-flow gap above for free). Full plan in `docs/delayed-return-swap.md`. **In progress**: all backend swap triggers done - delayed/overdue auto-swap (decisions 1, 3, 5), condition-change triggers on `reported_usable`/`reported_unusable`/`missing` (decision 6), and `Update`'s universal conflict-path swap covering both plain date edits and the copy flow (decision 7). Integration tests passing. Remaining: "no swap available" notification (decision 4) and the frontend (delay-preview endpoint, `ReturnChecklist.svelte` preview, booking-detail warning section).
