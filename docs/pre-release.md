...
- [ ] Web header logo - fetch `logo_url` from group settings and render it when present. **Placement decided:** dashboard root (`/`), inline with the primary CTA row - not the top nav. `web/src/routes/+page.svelte`'s CTA row was restructured (see below) to put the group name/switcher on the right of that row (primary CTAs on the left) specifically to reserve this spot; the logo replaces/joins that slot once built, not a separate nav-bar treatment.
- [ ] Logo upload UI - backend is fully built (`api/internal/handler/group_logo.go`, `group_settings.logo_file_id`, serves `/public/groups/{groupId}/logo` and `logo.png`, `logo_url` already in the settings API response) but there is no frontend form to upload/replace/delete it anywhere. Needs a file upload control in the group settings page (see "Settings page rename" below for where).
