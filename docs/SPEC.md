...
- **UPDATE**: Renamed to `TestApprovalFlow` with subtests for each access level × approval level combination. Tests now book for specific teams to verify per-team approval logic. `high` always needs approval including managers.
...
- **UPDATE**: Renamed to `TestAccess_TeamBookingVisibility`. Added `TestAccess_ViewOnlyRestrictions` for view-level users (can browse/report, cannot book/create/manage).
...
#### Step 2: Notifications (steps 1–6 done, 7–9 pending)
See [notifications.md](docs/notifications.md) for the full design.

Done:
- Migration: `users.max_access_level`, `users.notification_prefs` (JSONB), `group_settings` SMTP fields, `group_settings.notification_defaults` (JSONB), `notification_log` table ✅
...
- Notification preferences UI on profile page: semantic table with Bokningar/Ärenden groups, columns driven by `notification_channels` from group settings, inheritance hints, "Återställ" button ✅

Pending:
- Notifier interface + SMTPNotifier (SMTP credentials: per-group DB fields → system env fallback)
- Email templates (Go `html/template`, `_sv.html`/`_en.html` per event type)
- Event-triggered sends (fire-and-forget goroutines): booking_needs_approval, booking_confirmed, booking_rejected, booking_cancelled, issue_created, issue_assigned_to_me, issue_resolved, issue_commented
- Scheduled jobs: daily reminders (`start_date = tomorrow`) and overdue alerts (`end_date < today`), deduplicated via `notification_log`
